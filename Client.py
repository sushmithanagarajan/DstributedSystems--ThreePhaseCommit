...
def receive():
    while True:
        print "did i come 11111?"
        received_data = client.recv(size)
        print "data is here"
        print received_data
        #if "User :" in received_data:
        disbox.insert(END,received_data)
...
def sendmsg():                                                                                                       #function definition for send button
    ...
    print delta_time
    delta_time1 = delta_time.split(".",1)[0]
    print delta_time1
    #time difference between start time and end time, roundoff the value
    total_msg = "[User : "+login_msg + "]  " +str(delta_time1)+ " " + msg                                                   #User name and the time diff , msg is printed
...
def login():                                                                                                            #function for Login
    ...
    MSG = "User : " +login_msg+ " has entered the chatroom"
    client.send(MSG)#get the msg
    ...
    with open("participant_"+login_msg+"f1.txt",'w+') as f:
             f.read()
    print "i came till here"  # connect the host and port
...
def abort():
      datw1 = "Abort"
      client.send(datw1)
     # dt1 = client.recv(size)
      #print dt1
      #disbox.insert(END,dt1)


def commit():
    datw2 = "Commit"
    client.send(datw2)
    #dt2 = client.recv(size)
    #print dt2
    #disbox.insert(END , dt2)
...
top.title("Participant")                                                                                                #title for the Tkinter GUI
...
abort_button = Button(typframe,text = "Abort",command=abort)                                                         #create exit button
abort_button.pack()

commit_button = Button(typframe,text = "Commit",command=commit)                                                         #create exit button
commit_button.pack()
